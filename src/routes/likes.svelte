<script lang="ts">
	...
	import { LIKED_IDEAS_STORE, TOKEN_STORE } from "$lib/core/stores";
	...
	function loadMore() {
		if (!hasMore) return;
		const previousCount = $LIKED_IDEAS_STORE.length;
		Commands.appendLikedIdeas(18, cnt, () => {
			if ($LIKED_IDEAS_STORE.length < previousCount + 18) hasMore = false;
			cnt += 18;
		});
	}

	onMount(() => {
		Commands.clearLikedIdeas();
		...
		unsubscribe = TOKEN_STORE.subscribe((ts) => {
			if (!ts) {
				...
				Commands.clearLikedIdeas();
				Commands.loadLikedIdeas(cnt, 0, () => {
					if ($LIKED_IDEAS_STORE.length < cnt) hasMore = false;
				});
			}
		});
	});
...
<template>
	<div class="container-fluid p-3 card shadow border">
		...
		<!-- Items -->
		<div class="row">
			{#each $LIKED_IDEAS_STORE as idea}
				{#if tileMode}
