<script lang="ts">
	...
	import { RECENT_IDEAS_STORE } from "$lib/core/stores";
	...
	function loadMore() {
		if (!hasMore) return;
		const previousCount = $RECENT_IDEAS_STORE.length;
		Commands.appendRecentIdeas(18, cnt, () => {
			if ($RECENT_IDEAS_STORE.length < previousCount + 18) hasMore = false;
			cnt += 18;
		});
	}
	...
	onMount(() => {
		...
		Commands.loadRecentIdeas(cnt, 0, () => {
			if ($RECENT_IDEAS_STORE.length < cnt) hasMore = false;
		});
	});
</script>
