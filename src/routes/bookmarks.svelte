<script lang="ts">
	...
	import { SAVED_IDEAS_STORE, TOKEN_STORE } from "$lib/core/stores";
	...
	function loadMore() {
		if (!hasMore) return;
		const previousCount = $SAVED_IDEAS_STORE.length;
		Commands.appendSavedIdeas(18, cnt, () => {
			if ($SAVED_IDEAS_STORE.length < previousCount + 18) hasMore = false;
			cnt += 18;
		});
	}

	onMount(() => {
		Commands.clearSavedIdeas();
		...
		unsubscribe = TOKEN_STORE.subscribe((ts) => {
			if (!ts) {
				...
				Commands.clearSavedIdeas();
				Commands.loadSavedIdeas(cnt, 0, () => {
					if ($SAVED_IDEAS_STORE.length < cnt) hasMore = false;
				});
			}
		});
	});
...
<template>
	<div class="container-fluid p-3 card shadow border">
		...
		<!-- Items -->
		<div class="row">
			{#each $SAVED_IDEAS_STORE as idea}
				{#if tileMode}
