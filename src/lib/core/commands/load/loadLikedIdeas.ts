...
/**
 * Loads IdeaPreviews of liked ideas into the LIKED_IDEAS_STORE
 ...
 */
export function loadLikedIdeasCommand(limit = 0, offset = 0, then: () => void = null): void {
	...
	const success = (data: { entries: RAW_IdeaPreview[] }) => {
		LIKED_IDEAS_STORE.set(data.entries.map((e) => mapIdeaPreview(e)));
