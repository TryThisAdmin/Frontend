...
/**
 * Loads IdeaPreviews of own ideas into the OWN_IDEAS_STORE
 ...
 */
export function loadOwnIdeasCommand(limit = 0, offset = 0, then: () => void = null): void {
	...
	const success = (data: { entries: RAW_IdeaPreview[] }) => {
		OWN_IDEAS_STORE.set(data.entries.map((e) => mapIdeaPreview(e)));
