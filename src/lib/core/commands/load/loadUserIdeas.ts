...
import { ERROR_MESSAGE_STORE, TOKEN_STORE, USER_IDEAS_STORE } from "../../stores";

/**
 * Loads IdeaPreviews of a users ideas into the USER_IDEAS_STORE
 * @param uid User ID to load ideas from
 ...
 */
export function loadUserIdeasCommand(
	uid: string,
	limit = 0,
	offset = 0,
	then: () => void = null
	...
	const success = (data: { entries: RAW_IdeaPreview[] }) => {
		USER_IDEAS_STORE.set(data.entries.map((e) => mapIdeaPreview(e)));
