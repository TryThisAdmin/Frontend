...
/**
 * Appends IdeaPreviews to the TOP_IDEAS_STORE
 ...
 */
export function appendTopIdeasCommand(limit = 0, offset = 0, then: () => void = null): void {
	...
	const success = (data: { entries: RAW_IdeaPreview[] }) => {
		TOP_IDEAS_STORE.update((ideas) => [...ideas, ...data.entries.map((e) => mapIdeaPreview(e))]);
