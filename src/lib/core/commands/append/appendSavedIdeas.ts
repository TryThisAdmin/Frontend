...
/**
 * Appends IdeaPreviews to the SAVED_IDEAS_STORE
 ...
 */
export function appendSavedIdeasCommand(limit = 0, offset = 0, then: () => void = null): void {
	...
	const success = (data: { entries: RAW_IdeaPreview[] }) => {
		SAVED_IDEAS_STORE.update((ideas) => [...ideas, ...data.entries.map((e) => mapIdeaPreview(e))]);
