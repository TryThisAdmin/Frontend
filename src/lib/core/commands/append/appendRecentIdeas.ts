...
/**
 * Appends IdeaPreviews to the RECENT_IDEAS_STORE
 ...
 */
export function appendRecentIdeasCommand(limit = 0, offset = 0, then: () => void = null): void {
	...
	const success = (data: { entries: RAW_IdeaPreview[] }) => {
		RECENT_IDEAS_STORE.update((ideas) => [...ideas, ...data.entries.map((e) => mapIdeaPreview(e))]);
