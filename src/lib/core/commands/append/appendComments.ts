...
/**
 * Appends Comments to the COMMENTS_STORE
 * @param idea Idea ID to load comment from
 ...
 */
export function appendCommentsCommand(
	idea: string,
	limit = 0,
	offset = 0,
	then: () => void = null
	...
	const success = (data: { entries: RAW_Comment[] }) => {
		COMMENTS_STORE.update((comments) => [...comments, ...data.entries.map((e) => mapComment(e))]);
